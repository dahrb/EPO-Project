...
class Experiments_Nested():
    ...
    def training_ngram_core(self,param,steps,y_train):
        ...
        if self.experiment == '2':
            ...
            nested_scores = cross_validate(CV, X= self.X_train, y=y_train, scoring=self.scoring, cv=RepeatedStratifiedKFold(n_splits=self.cv_num_out, n_repeats=self.repeat, random_state=42), return_train_score=True,n_jobs=-1)          
    ...
    def training_w2v_core(self, param, steps, y_train):
        ...
        if self.experiment == '2':
            ...
            nested_scores = cross_validate(CV, X= self.X_train, y=y_train, scoring=self.scoring, cv=RepeatedStratifiedKFold(n_splits=self.cv_num_out, n_repeats=1, random_state=42), return_train_score=True,n_jobs=-1)          
