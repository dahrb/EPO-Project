...
class Experiments_Nested():

    def __init__(self,models=[], experiment = '1',opposition=False, cv_num_in = 3, cv_num_out = 10, num_iter = 100, train_both=False, repeat = 1, no_grid = False, input_representation = 'N-Gram'):
    ...
    def training_ngram_core(self,param,steps,y_train):
        ...
        if self.experiment == '2':
            nested_scores = cross_validate(CV, X= self.X_train, y=y_train, scoring=self.scoring, cv=TimeSeriesSplit(n_splits=self.cv_num_out), return_train_score=True, n_jobs=-1)          
        
        else:
            nested_scores = cross_validate(CV, X= self.X_train, y=y_train, scoring=self.scoring, cv=RepeatedStratifiedKFold(n_splits=self.cv_num_out, n_repeats=self.repeat, random_state=42), n_jobs=-1, return_train_score=True)          
        
        time_overall = time.time() - time_taken
        print("done in %0.3fs" % (time_overall))

        self.results.append(nested_scores)
    ...
    def training_w2v_core(self, param, steps, y_train):
        ...
        if self.experiment == '2':
            nested_scores = cross_validate(CV, X= self.X_train, y=y_train, scoring=self.scoring, cv=TimeSeriesSplit(n_splits=self.cv_num_out), n_jobs=-1, return_train_score=True)          
        
        else:
            nested_scores = cross_validate(CV, X= self.X_train, y=y_train, scoring=self.scoring, cv=RepeatedStratifiedKFold(n_splits=self.cv_num_out, n_repeats=1, random_state=42), n_jobs=-1, return_train_score=True)          
...
class Hyperparameters(Experiments_Nested):
